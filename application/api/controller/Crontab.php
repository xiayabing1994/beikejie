        ...
        foreach($goods_arr as $goods){
            ...
            foreach($goodsinfo['spec'] as $specitem){
                ...
                if($specitem['stock_num']>0) {
                    ...
                    User::money($specitem['stock_num']*$ordergoods['goods_price'],$goods['user_id'],'dont sale return money');
                    User::quota($specitem['stock_num']*$ordergoods['goods_quota'],$goods['user_id'],'dont sale return quota');
                }
            }
        }
        //本期结束
//        $this->_whole->where('w_period','curr')->update(['w_period'=>'pass']);
        //下期开始
//        $this->_whole->where('w_period','next')->update(['w_period'=>'curr','start'=>time(),'end'=>time()+86400*5]);
        Db::commit();

        //1.更改wholesale所有当前期记录peroid为pass
        //2.更改当前期中所有商品状态为下架状态
        //3.查询当前期中所有商品,计算出未卖出商品数量,返还用户配额和余额
        //4.更改下期商品状态为当期，所有已挂售商品状态为上架，更改所有记录开始时间与结束时间
    }
}